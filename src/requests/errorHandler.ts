...
export default (err: AxiosError, funcName: string): void => {
    if (err.response) {
        console.error(`Response error on ${funcName}. The request was made and the server responded.`);
        ...
        console.error(`Response error on ${funcName}. The request returned but no response received.`);
